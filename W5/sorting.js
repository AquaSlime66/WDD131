...
function SearchList(list, a){

    function searchCallBack(ob){
        
        return ( ob.name.toLowerCase().includes(a.toLowerCase()) || ob.description.toLowerCase().includes(a.toLowerCase()) ||           ob.tags.find((tag) => tag.toLowerCase().includes(a.toLowerCase())) 
    );
        //if includes the string, true, if not, false
    }
    return list.filter(searchCallBack);
    //filter expects a list of true/false values
}
