...
function renderTasks(tasks) {
  ...
  const myBigList = document.querySelector('#todoList');
  myBigList.innerHTML = "";
  // loop through the tasks array. transform (map) each task object into the appropriate HTML to represent a to-do.\
  const html = tasks.map(taskTemplate).join("");
  // console.log(html);
  myBigList.innerHTML = html;
}

function newTask() {
  // get the value entered into the #todo input
  const myTodo = document.querySelector("#todo").value;
  // const newTodo = { detail: myTodo, completed: false};
  // add it to our arrays tasks
  tasks.push({ detail: myTodo, completed: false });
...
// const theBoss = document.getElementById("submitTask");
// theBoss.addEventListener("click", newTask);
